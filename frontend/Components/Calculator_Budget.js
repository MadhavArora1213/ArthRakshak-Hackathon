...
import { View, Text, TouchableOpacity, StyleSheet, Dimensions, Animated, Easing, Modal, ScrollView, TextInput, Alert } from 'react-native';
...
const Calculator = () => {
  ...
  const calculateBudgetTotals = () => {
    ...
    const savingsPercentValue = parseInt(budgetData.savingsPercentGoal.replace('%', ''));
    ...
    setBudgetPersonality(personality);
    ...
    >
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerControls}>
          <TouchableOpacity
            onPress={() => setCurrentMode(currentMode === 'budget' ? 'calculator' : 'budget')}
            style={[styles.iconButton, { backgroundColor: currentMode === 'budget' ? currentTheme.buttonPrimary : currentTheme.buttonNeutral }]}
          >
            <Ionicons 
              name={currentMode === 'budget' ? 'calculator-outline' : 'wallet-outline'} 
              size={20} 
              color={currentMode === 'budget' ? '#ffffff' : currentTheme.buttonText} 
            />
          </TouchableOpacity>
